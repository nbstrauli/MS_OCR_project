...
#TODO: keep a log of delete cdr3
...
def mixcr_to_csv(mixcr_file, trb_or_igh):
    ...
    output_filename = os.path.join(os.path.dirname(mixcr_file_dir), '_'.join(['mixcr',trb_or_igh ,'extracted']), mixcr_file_base.replace(".txt",".csv"))
    ...
    for row in mixcr_reader:
         ...
         if vgene and jgene and len(cdr3) > 7 and not ('*' in cdr3 or '~' in cdr3 or '_' in cdr3):
                result = [trim_name_suffix(vgene), trim_name_suffix(jgene), trim_name_suffix(dgene), cdr3, count, barcode]
