...
from barcode import create_barcode_json
...
# basically, remove the directory and remove the redudancy in file and dir names
    ...
    for root, dirs, files in os.walk(RAW_DATA_ROOT):
        ...
        # Auto_user_SN2-48-Template-TCR_sequencing_84_055
            #parent_dir  = basename(dirname(dirname(root))).split('Auto_user_SN2-')[-1]
            # DEBUG: Split by SN2- only for Klaus's LGI1 plates otherwise split by Auto_user_SN2-
            parent_dir  = basename(dirname(dirname(root))).split('SN2-')[-1]
        ...
        # make new dir
            ...
            for analysis_dir in [ 'raw_fastq',
                 'mixcr_igh', 'mixcr_igh_full', 'mixcr_igh_extracted', 'mixcr_igh_export_align', 'shm',
                 'mixcr_trb', 'mixcr_trb_full', 'mixcr_trb_extracted',
                ]:
                abs_analysis_dir = join(abs_dir, analysis_dir)
                if not os.path.exists(abs_analysis_dir):
                    os.makedirs(abs_analysis_dir)

    logging.info( "Total number of seq run dir is %s" , counter)
